...
export class RootUserAccessKeysActiveAnalyzer extends BaseAnalyzer {
    ...
    analyze(params: any, fullReport?: any): any {
        ...
        const root_user_access_keys_active: CheckAnalysisResult = { type: CheckAnalysisType.Security };
        root_user_access_keys_active.what = "Are there any root user access keys active?";
        root_user_access_keys_active.why = "Root user access keys are unrestrictable hence shouldn't be used as damage will be determental if they gets leaked"
        ...
        if (credentials[rootUserIndex].access_key_1_active === 'false' && credentials[rootUserIndex].access_key_2_active === 'false') {
            ...
            analysis.action = 'Delete the root user access keys instead use IAM user access keys for programmatic access';
        }
