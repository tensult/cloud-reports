    ...
    public analyze(params: any, fullReport?: any): any {
        ...
        for (const region in allDomains) {
            ...
            for (const instances of regionReservedInstances) {
                ...
                const Time = this.getInstancesRenewTime(StartTime, Duration);
                r_Instances_Analysis.resourceSummary = {
                    name: "ReservedInstances",
                    value: `${ReservationName} | ${State}`,
                };
                if (Time >= millsIn30Days) {
                ...
                } else if (Time == millsIn30Days) {
                    r_Instances_Analysis.severity = SeverityStatus.Warning;
                    r_Instances_Analysis.message = `Ensure that your AWS Elasticsearch Reserved Instances (RIs)
                     are renewed before expiration in order to get a significant discount on the hourly charges.`;
                }
                else {
                    ...
                    r_Instances_Analysis.title = Charges;
                    r_Instances_Analysis.action = `If you don't wanna pay for the use of service kindly renew your service.` ;
                }
