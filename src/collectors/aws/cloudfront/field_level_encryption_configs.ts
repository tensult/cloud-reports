...
export class  FieldLevelEncryptionconfigsCollector extends BaseCollector {
    ...
    private async listAllFieldLevelEncryptionconfigs() {
        try {
            ...
            const field_level_encrypt_Data = await CollectorUtil.cachedCollect(field_level_encrypt_Collector);
            ...
            for (const field_level_encrypt of field_level_encrypt_Data.field_level_encrypt) {
                const cloudfrontFieldLevelEncryptionData:
                    AWS.CloudFront.GetFieldLevelEncryptionConfigResult =
                    await cloudfront.getFieldLevelEncryptionConfig({ Id: field_level_encrypt.Id }).promise();
