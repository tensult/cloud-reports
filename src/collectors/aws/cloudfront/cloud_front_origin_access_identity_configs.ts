...
export class CloudFrontOriginAccessIdentityConfigsCollector extends BaseCollector {
    ...
    private async listAllCloudFrontOriginAccessIdentityConfigs() {
        try {
            ...
            cloud_front_origin_access_identity_Collector.setSession(this.getSession());
            const cloud_front_origin_access_identity_Data = await CollectorUtil.cachedCollect(cloud_front_origin_access_identity_Collector);
            ...
            for (const cloud_front_origin_access_identity of cloud_front_origin_access_identity_Data.cloud_front_origin_access_identity) {
                const Cloud_Front_Origin_Access_Identity_Data:
                    AWS.CloudFront.GetCloudFrontOriginAccessIdentityResult =
                    await cloudfront.getCloudFrontOriginAccessIdentityConfig({ Id: cloud_front_origin_access_identity.Id }).promise();
                    cloud_front_origin_access_identity_configs[cloud_front_origin_access_identity.Id] = Cloud_Front_Origin_Access_Identity_Data.CloudFrontOriginAccessIdentity;
